...
public class KhachHangBUS {
    ...
    public Boolean add(KhachHangDTO kh) {
        ...
        if(check) this.listKhachHang.add(kh);
        return check;
    }
    
    public Boolean delete(KhachHangDTO kh) {
        boolean check = khDAO.delete(Integer.toString(kh.getMaKH())) != 0;
        if(check) this.listKhachHang.remove(getIndexByMaDV(kh.getMaKH()));
        return check;
    }
    
    public Boolean update(KhachHangDTO kh) {
        boolean check = khDAO.update(kh) != 0;
        if(check) this.listKhachHang.set(getIndexByMaDV(kh.getMaKH()), kh);
        return check;
    }
